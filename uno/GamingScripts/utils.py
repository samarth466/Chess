...
def gen_deck() -> dict[str, Card]:
    ...
    'O4a': (Card(None, 100, 400, 'Orange 4', 4, pygame.Color(*ORANGE)), COLORS[BLUE]+'Rb'),
        ...
        'WIad': (Card(None, 100, 400, 'Wild Dark', None, None), COLORS[BLUE]+'Ra'),
        'WIbd': (Card(None, 100, 400, 'Wild Dark', None, None), COLORS[RED]+'Sb'),
        'WIcd': (Card(None, 100, 400, 'Wild Light', None, None), COLORS[YELLOW]+'1a'),
        'WIdd': (Card(None, 100, 400, 'Wild Dark', None, None), COLORS[YELLOW]+'Rb'),
...
def shuffle_deck(original_deck: dict[str, Card]): > deque:
    shuffled_deck = DictLifoQueue(112)
    while original_deck:
...
def create_key(card: Card):
    if Card.role == "Wild Color Draw":
        return "WDCD"
    elif Card.role == "Wild Dark":
        return "WIdd"
    elif "Flip" in Card.role:
        return f"{COLORS[(card.color.r,card.color.g,card.color.b)]}Fb"
    elif "Reverse" in card.role:
        return f"{COLORS[(card.color.r,card.color.g,card.color.b)]}Rb"
    elif "Draw 5" in card.role:
        return f"{COLORS[(card.color.r,card.color.g,card.color.b)]}D5b"
    elif "Skip Everyone" in card.role:
        return f"{COLORS[(card.color.r,card.color.g,card.color.b)]}SEb"
    elif card.role == "Wild Draw 2":
        return "WD2d"
    elif card.role == "Wild Light":
        return "WIdl"
    else:
        return f"{COLORS[(card.color.r,card.color.g,card.color.b)]}{card.number}b"

def bottom_card(card: Card, deck: DictLifoQueue):
    new_deck = DictLifoQueue(112)
    cards = []
    while not deck.empty:
        cards.append((*deck.get()))e3w
    key = create_key(112)
    if key in deck:
        if key.endswith("b"):
            deck[key[:-1]+"a"] = deck[key]
        elif key.endswith("d"):
            deck[key[:-1]+"a"] = deck[key[:-1]+"b"]
            deck[key[:-1]+"b"] = deck[key[:-1]+"c"]
            deck[key[:-1]+"c"]
...
class Player:
    ...
    def __init__(self, id: int, username: str, hand: DictLifoQueue):
        self.username = username
        self.hand = Hand([cards.pop() for _ in range(7)])
